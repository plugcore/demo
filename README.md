...
## Requisitos
- Nodejs 10.16 o superior con npm (se instala por defecto cuando instalas Nodejs)
- Git
...
```
git clone https://github.com/plugdata/plugdata-demos.git
```
...
Ahora que ya lo tenemos instalado ya lo podemos poner en marcha, hacer tests, etc. Los comandos que hay son:
...
- `src`: Contiene todo el código ejecutable del proyecto, como controladores, servicios, etc.
  ...
  - `vehicle`: Esta es una carpeta de tipo entidad, es decir que aquí habrá todos los *.ts que tengan que ver con vehículos.
    ...
    - `vehicle.api.ts`: Aquí tendremos todos los modelos que se refieran a la API, es decir que indiquen que Requests y que Responses vamos a tener, que parámetros etc. Todas las clases que definamos aquí nos serviran para 3 cosas:
      1. __Para validar todas las request__. Es decir que si indicamos que el objeto de entrada debe tener cierta propiedad de tipo string y que sea obligatoria, el sistema lo hará automáticamente por nostros.
      ...
      3. __Tipificar el controlador__. Esto ya es algo más própio de Typescript, pero todo lo que haya aquí nos servirá para ayudarnos en los controladores y tener más controlados los tipos de entrada/salida.
    - `vehicle.controller.ts:` Es una clase de tipo controlador que define el prefijo de url, por ejemplo "/vehicle", y sobre el cual vamos a agregar tódos los métodos HTTP que queramos. Ahora está puesto un ejemplo de RESTFUL con operaciones CRUD.
...
Para empezar es recomendable hacer un `POST` a `http://localhost:3000/vehicle` con un objeto del estilo:
```
{
    "model": "Tesla S1",
    "year": 2019
...
Muchas gracias por probar, si encuentras cualquier problema por favor deja abre un ticket en: [https://github.com/plugdata/plugdata-framework/issues](https://github.com/plugdata/plugdata-framework/issues)
